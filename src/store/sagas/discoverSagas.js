...
function* fetchDiscover(action) {
    ...
    if(action.payload.type === 'movie') {
        ...
        const result = yield axios.get(`https://api.themoviedb.org/3/discover/${action.payload.type}?api_key=${API_KEY}&language=en-US&sort_by=${action.payload.sort}&include_adult=false&include_video=false&page=${action.payload.page}&first_air_date_year=${action.payload.year}&with_keywords=${keywords}&with_genres=${genres}`)
        .then(res => res.data);
